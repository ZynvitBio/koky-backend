...
module.exports = {
  ...
  bootstrap({ strapi }) {
    ...
    process.nextTick(async () => {
      try {
        const existingFaq = await strapi.documents('api::faq.faq').findFirst({
          filters: { topic: 'Contenido de proteína de los tofus y leche de soya' },
          status: 'published'
        });
        
        if (!existingFaq) {
          strapi.log.info('🌱 Protein FAQ missing. Seeding it...');
          ...
          strapi.log.info('🌱 Successfully seeded Protein FAQ.');
        }
      } catch (err) {
